...
function TravelBlogSection() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const res = await api.get("/admin-blogs"); // Fetch all blogs
        setBlogs(res.data);
        setLoading(false);
      } catch (err) {
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-center mt-[80px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
        {blogs.map((blog) => {
          ...
          // Limit title to 2 lines (~80 chars) and description to 4 lines
          const titlePreview = title.length > 80 ? title.slice(0, 77) + "..." : title;
          const descriptionPreview = description.replace(/<[^>]+>/g, ""); // Remove HTML tags
          const descriptionLimited = descriptionPreview.length > 150 ? descriptionPreview.slice(0, 147) + "..." : descriptionPreview;

          return (
            <Link
              to= {`/blog/${id}`} // Replace '#' with `/blog/${id}` when page is ready
              key={id}
